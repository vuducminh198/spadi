...
<template>
    <div style="padding:10px; background-color:white;">
        <div style="font-size:18px; font-family: 'Open Sans'; margin-bottom:10px;">
            TẠO CHI NHÁNH {{$store.state.adminSelectedChain.name}}
        </div>
        <div style="padding:10px; background-color:white; padding-top:40px; padding-bottom:40px;">
                                        ...
                                        <el-tab-pane label="Điện thoại">
                                            <template v-for="item,index in form.addShop.phone">
                                                <el-row :gutter="20" v-if="typeof item!=='undefined'"
                                                    ...
                                                    <el-col :md="20" :xs="18">
                                                        <el-input placeholder="Nhập số điện thoại" :value="item"
                                                                  disabled>
    ...
    export default {
        ...
        methods: {
            m_getInfoCurrentLocation() {
                this.$http.get("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + this.v.markers[0].position.lat + "," + this.v.markers[0].position.lng + "&sensor=false&key="+this.GMAPKEY())
                    .then(data => {
