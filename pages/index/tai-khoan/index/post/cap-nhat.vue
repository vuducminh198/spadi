...
<template>
    ...
    <div style="padding:10px; background-color:white;">
        ...
        <form @submit="m_formUpdatePost($event)">
            <div>
...
<script>
    ...
    export default {
        ...
        methods: {
            m_getOldData() {
                this.$http.get(process.env.API.Post_GetPublicByData + this.$route.query._)
                    .then(data => {
                        console.log(data);
                    })
                    .catch(error => {
            ...
            m_formUpdatePost(e) {
                ...
                postForm.append('chain', this.$store.state.adminSelectedChain._id);
                ...
                this.$confirm(`Bạn có chắc muốn thêm bài viết: ${this.form.addPost.title} choc cửa hàng ${this.$store.state.adminSelectedChain.name} không?`, 'Chú ý', {
                    ...
                    .then(() => {
                        this.isLoading = true;
                        this.$http.post(process.env.API.Post_Create, postForm, this.cf())
                            .then(data => {
